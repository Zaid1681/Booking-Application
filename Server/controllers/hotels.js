...
export const createHotel = async (req, res, next) => {
  ...
  try {
    ...
    console.log("Hotel created Sucess");
  } catch (error) {
    next(error);
...
export const updateHotel = async (req, res, next) => {
  try {
    const updatedHotel = await Hotels.findByIdAndUpdate(
      req.params.id,
      { $set: req.body }, //findByIdAndUpdate -> sends the previous doc not the updated one
      { new: true } // send new version of doc
    );
...
export const deleteHotel = async (req, res, next) => {
  try {
    ...
    console.log("Hote deleted sucess");
  } catch (error) {
...
export const getAllHotel = async (req, res, next) => {
  ...
  try {
    ...
    console.log("Hotel fetched sucess");
  } catch (error) {
    res.status(500).json(error);
    //need to understand how usnig express middleware
  }
...
export const countByCity = async (req, res, next) => {
  ...
  try {
    const list = await Promise.all(
      cities.map((city) => {
        return Hotels.countDocuments({ city: city }); //mogoDb doc which counts
      })
    );
